...
using CBPlatformTest.Core;
using CBPlatformTest.Core.Effects;
using CBPlatformTest.Core.Inputs;
...
namespace CBPlatformTest.Screens;
...
class MainMenuScreen : MenuScreen
{
    ...
    private void SetLanguageText()
    {
        ...
        Title = "CBPlatformTest"; // TODO uncomment this if you want it to use Resources.MainMenu; instead
    }
    ...
    public override void LoadContent()
    {
        ...
        settingsManager ??= ScreenManager.Game.Services.GetService<SettingsManager<CBPlatformTestSettings>>();
    ...
    private void UpdateTutorialSteps(GameTime gameTime)
    {
        if (gameTime.TotalGameTime - timeSinceLastMessage > TimeSpan.FromSeconds(3)) // Should the showtime be in settings?
        {
            ...
            if (CBPlatformTestGame.IsMobile)
            {
                if (tutorialStep > 3)
                {
                    tutorialStep = -1;
